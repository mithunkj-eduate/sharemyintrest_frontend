...
const Home = () => {
  ...
  const feactLike = async (id) => {
    try {
      const res = await axios.put(`${BASEURL}/post/like`, { postId: id }, config);
      const resData = res.data.data;
      setAllPost((prev) =>
  ...
  const feactunLike = async (id) => {
    try {
      const res = await axios.put(`${BASEURL}/post/unlike`, { postId: id }, config);
      const resData = res.data.data;
      setAllPost((prev) =>
        prev.map((post) => (post._id === resData._id ? resData : post))
      );
  ...
  const handleShareSubmit = async () => {
    try {
      ...
      const res = await axios.post(`${BASEURL}/messages/message`, formdata, config);
      if (res.status === 200) {
    ...
    <div className="container feed-container">
              ...
              item.postedBy ? (
                <div key={item._id} className="feed-post">
                  <div className="feed-header d-flex align-items-center">
                    ...
                    >
                      <img
                        ...
                        loading="lazy"
                      />
                      <span className="fw-bold">{item.postedBy?.userName}</span>
                    </NavLink>
                  </div>
                  ...
                  <div className="feed-body">
                    ...
                    <p className="text-muted small">{item.views.length} views</p>
      ...
      <Modal show={showShareModal} onHide={handleCloseShare} centered>
        ...
        <Modal.Body>
          ...
          />
          <p className="text-muted small mb-3">Select friends to share with:</p>
          {following.length > 0 ? (
            following.map((item) => (
              <div
                key={item._id}
              ...
              >
                {selectedIds.length ? 
                <FaCircleCheck
                  className={`fs-5 ${selectedIds.includes(item._id) ? "text-primary" : "text-muted"}`}
                />: null}
                <img
